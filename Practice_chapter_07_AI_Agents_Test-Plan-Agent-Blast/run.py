...
import navigation
from tools.errors import (AgentError, InvalidKeyError, JiraAuthError,
                          JiraNotFoundError, JiraRateLimitError, LLMError,
                          NotPlannableError, SchemaError)
...
def main():
    ...
    ap.add_argument("prompt", nargs="*",
                    help='e.g. SCRUM-42, or "make a plan for SCRUM-42"')
    ap.add_argument("--health", action="store_true",
                    help="test both connections and exit")
    ap.add_argument("--dry-run", action="store_true",
                    help="fetch and normalize, but never call the LLM")
    ap.add_argument("--force", action="store_true",
                    help="plan even if the ticket is thin")
    args = ap.parse_args()
