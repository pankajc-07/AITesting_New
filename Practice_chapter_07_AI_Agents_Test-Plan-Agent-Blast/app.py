"""Test Plan Agent - main UI (Streamlit).

One prompt box: "Fetch SCRUM-42 and create a test plan".
Built on the B.L.A.S.T. protocol with the A.N.T. 3-layer architecture.
"""
import sys
...
st.caption("Jira ticket in, formal test plan out. Built on B.L.A.S.T. + Groq.")

cfg = config_store.load_config()
missing = config_store.missing_credentials()
if missing:
    st.warning(f"Not configured yet: **{', '.join(missing)}**. "
               "Open **Settings** in the sidebar to add them.")

with st.sidebar:
    st.subheader("⚡ Connection")
    st.write(f"**Jira:** {cfg['jira_url'] or '_not set_'}")
    _mdl = cfg.get("groq_model") or "?"
    st.write(f"**Groq:** `{_mdl}`")
    st.caption("Test connections on the Settings page.")
    st.divider()
    st.subheader("⚙️ Options")
    force = st.checkbox("Plan anyway if ticket is thin", value=False,
                        help="By default, the agent refuses tickets that score below "
                             "5/11 on readiness and returns a gap report.")

default_prompt = (f"Fetch {cfg['default_jira_key']} and create a test plan"
                  if cfg.get("default_jira_key")
                  else "Fetch PROJ-1 and create a test plan")
...
if go:
    ...
    for name in STEPS:
        slots[name].markdown(f"◻ {name}")

    def progress(step, status, detail=""):
        icon = {"running": "⏳", "done": "✅", "fail": "❌"}.get(status, "◻")
        extra = f" — {detail}" if detail else ""
        slots[step].markdown(f"{icon} **{step}**{extra}")

    try:
        with st.spinner("Working..."):
            result = navigation.run(prompt, progress=progress, force=force)
        st.session_state["result"] = result
        st.success(f"✅ Test plan saved to `{result['path'].name}`")
        ...
        if rd["gaps"]:
            ...
            for g in rd["gaps"]:
                st.markdown(f"- {g}")
        st.info("This is correct behavior (BR-4). Tick 'Plan anyway' in sidebar to override.")
